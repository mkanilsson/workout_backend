...
impl Token {
    ...
    // TODO: Only find valid tokens (expire)
    pub async fn find_by_id(db: &Pool<MySql>, id: String) -> Result<Option<Self>> {
        Ok(
            sqlx::query_as!(Token, "SELECT * FROM tokens WHERE id = ? LIMIT 1", id)
                .fetch_optional(db)
                .await
                .map_err(error::from_sqlx_error)?,
        )
    ...
    pub async fn find_by_value(db: &Pool<MySql>, value: &str) -> Result<Option<Self>> {
        Ok(
            sqlx::query_as!(Token, "SELECT * FROM tokens WHERE value = ? LIMIT 1", value)
                .fetch_optional(db)
                .await
                .map_err(error::from_sqlx_error)?,
        )
    ...
    pub async fn find_all_by_user_id(db: &Pool<MySql>, user_id: String) -> Result<Vec<Self>> {
        Ok(
            sqlx::query_as!(Token, "SELECT * FROM tokens WHERE user_id = ?", user_id)
                .fetch_all(db)
                .await
                .map_err(error::from_sqlx_error)?,
        )
